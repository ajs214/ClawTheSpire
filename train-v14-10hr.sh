#!/usr/bin/env bash
# train-v14-10hr.sh — V14 cold start, 10-hour run
#
# V14 changes:
...
#
# Usage:
#   nohup bash ~/AJS_CTS/ClawTheSpire/train-v14-10hr.sh > train-v14-10hr.log 2>&1 &
#   tail -f train-v14-10hr.log
#
...
echo "=== STS2 AlphaZero Training V14 — Cold Start, 10 Hours ==="
echo "  Duration cap:  10 hours (hard timeout)"
echo "  Gen budget:    2800"
echo "  Games/gen:     10"
echo "  MCTS sims:     600 base (progressive: 240→1080)"
echo "  Start:         FRESH (random weights)"
...
echo "Expected end:  $(date -v+10H 2>/dev/null || date -d '+10 hours' 2>/dev/null || echo '(10 hours from now)')"
echo "-----------------------------------"
...
(
    ...
    if kill -0 "$TRAIN_PID" 2>/dev/null; then
        echo ""
        echo "!! 10-hour cap hit — sending SIGTERM to training pid $TRAIN_PID"
...
if [ "$RC" -eq 143 ] || [ "$RC" -eq 137 ]; then
    echo "=== V14 10-hour cap reached at $(date) (exit $RC) ==="
elif [ "$RC" -eq 0 ]; then
    echo "=== V14 gen budget exhausted before 10-hour cap at $(date) ==="
else
