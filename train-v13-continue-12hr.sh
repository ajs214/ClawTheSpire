#!/usr/bin/env bash
# train-v13-continue-12hr.sh — 12-hour V13 continuation
#
# Resumes from latest V13 checkpoint.
# Same hyperparameters, updated simulator with v0.102/v0.103 patch fixes.
#
# Usage:
#   nohup bash ~/AJS_CTS/ClawTheSpire/train-v13-continue-12hr.sh > train-v13-continue-12hr.log 2>&1 &
#   tail -f train-v13-continue-12hr.log
...
echo "=== STS2 AlphaZero Training V13 — 12 Hour Continuation ==="
...
if [ "$RC" -eq 143 ] || [ "$RC" -eq 137 ]; then
    echo "=== V13 12-hour cap reached at $(date) (exit $RC) ==="
elif [ "$RC" -eq 0 ]; then
    echo "=== V13 gen budget exhausted before 12-hour cap at $(date) ==="
else
