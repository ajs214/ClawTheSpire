#!/usr/bin/env bash
# train-v12-continue-12hr.sh — Continue V12 training for another 12 hours
#
# Picks up from the latest checkpoint in alphazero_checkpoints_v12/.
# Adds 2400 more generations on top of whatever gen we're at (timeout
# is the real stopping condition).
#
# Usage:
#   nohup bash ~/AJS_CTS/ClawTheSpire/train-v12-continue-12hr.sh > train-v12-continue-12hr.log 2>&1 &
#   tail -f train-v12-continue-12hr.log
#
...
echo "=== STS2 AlphaZero Training V12 — 12 Hour Continuation ==="
echo "  Duration cap:  12 hours (hard timeout)"
echo "  Gen budget:    2400 (additional — timeout is the real stop)"
echo "  Games/gen:     10"
...
if [ "$RC" -eq 143 ] || [ "$RC" -eq 137 ]; then
    echo "=== V12 12-hour continuation cap reached at $(date) (exit $RC) ==="
elif [ "$RC" -eq 0 ]; then
