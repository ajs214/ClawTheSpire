#!/usr/bin/env bash
# train-v15-continue-20hr.sh — Continue V15 training, 20-hour run
#
# Continues from the latest V15 checkpoint.
#
# Usage:
#   nohup bash ~/AJS_CTS/ClawTheSpire/train-v15-continue-20hr.sh > train-v15-continue-20hr.log 2>&1 &
#   tail -f train-v15-continue-20hr.log
#
...
echo "=== STS2 AlphaZero Training V15 — Continue 20 Hours ==="
...
if [ "$RC" -eq 143 ] || [ "$RC" -eq 137 ]; then
    echo "=== V15 20-hour cap reached at $(date) (exit $RC) ==="
elif [ "$RC" -eq 0 ]; then
    echo "=== V15 gen budget exhausted before 20-hour cap at $(date) ==="
else
