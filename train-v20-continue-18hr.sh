#!/usr/bin/env bash
# train-v20-continue-18hr.sh — Continue V20 training for 18 more hours
#
# Resumes from the latest V20 checkpoint.
#
# Usage:
#   nohup bash ~/AJS_CTS/ClawTheSpire/train-v20-continue-18hr.sh > train-v20-continue-18hr.log 2>&1 &
#   tail -f train-v20-continue-18hr.log
...
if [ -z "$LATEST_CKPT" ]; then
    echo "ERROR: No V20 checkpoint found in $SAVE_DIR"
    exit 1
fi

echo "=== STS2 AlphaZero Training V20 — Continue 18 Hours ==="
...
if [ "$RC" -eq 143 ] || [ "$RC" -eq 137 ]; then
    echo "=== V20 18-hour continuation cap reached at $(date) (exit $RC) ==="
elif [ "$RC" -eq 0 ]; then
